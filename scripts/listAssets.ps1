...
#		Create report
#		Pull down report
# Version: .2 Modified October 26, 2017
...
function listSystems($reportFormat, $reportValue, $Reportchapter) {
  $pluginID = Read-Host -Prompt 'Plugin to search for ...'
...
function Download-Report($file) {
  ...
  Write-Host $object
  $response = Invoke-RestMethod -Method Get -Uri $Tenableio/$object -Header @{ "X-ApiKeys" = $headers } -OutFile tenable_io.csv
}
